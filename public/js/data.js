// public/js/data.js
// Static reference data: reading list and implementation suggestions engine.
// No API calls — pure data + computation.

export const BOOKS = [
  { phase: 'Phase 1 — Security Foundations', books: [
  ...
  { phase: 'Phase 2 — Offensive Security', books: [
  ...
  { phase: 'Phase 3 — AI & ML Engineering', books: [
  ...
  { phase: 'Phase 4 — Systems Architecture', books: [
  ...
  { phase: 'Phase 5 — Cloud & Infrastructure', books: [
  ...
  { phase: 'Phase 6 — Product & Business', books: [
...
export const ALL_BOOKS = BOOKS.flatMap(s => s.books.map(b => ({ ...b, phase: s.phase })));
...
/**
 * getSuggestions(book, notes)
 * Returns 4–7 specific implementation suggestions based on the book title
 * and keyword detection in the contributor's reading notes.
 */
export function getSuggestions(book, notes) {
...
// ── Architecture tracker task definitions ─────────────────────────────────────
export const ARCH_SERVICES = [
  { n: 'Scan Orchestrator',   ic: '🔍', d: 'Manages scan jobs, dispatches tasks, wraps Nmap/Nuclei/OpenVAS.',
    tasks: ['Define scan job data model', 'Design message queue schema', 'Document Nmap integration spec', 'Document Nuclei template strategy', 'Define scan result schema', 'Plan SSRF controls and scan target validation'] },
  { n: 'AI Analysis Engine',  ic: '🧠', d: 'Produces prioritised plain-language findings via LLM API.',
    tasks: ['Define prompt architecture', 'Design finding prioritization schema', 'Plan hallucination mitigation strategy', 'Define LLM provider abstraction interface', 'Design prompt injection controls', 'Define evaluation suite structure'] },
  { n: 'Asset Manager',       ic: '📡', d: 'Maintains asset inventory — networks, domains, IPs, endpoints.',
    tasks: ['Define asset data model', 'Plan asset discovery flow', 'Design subdomain enumeration approach', 'Define asset tagging schema', 'Plan multi-tenant data isolation'] },
  { n: 'Alert Engine',        ic: '🔔', d: 'Deduplication, correlation, suppression, routing.',
    tasks: ['Define alert severity taxonomy', 'Design deduplication logic', 'Plan correlation rules engine', 'Define suppression rules', 'Design alert routing configuration'] },
  { n: 'Compliance Engine',   ic: '📋', d: 'Maps posture to NDPA 2023, NIST CSF 2.0, ISO 27001.',
    tasks: ['Map NDPA 2023 control set', 'Map NIST CSF 2.0 controls', 'Design gap report schema', 'Plan evidence collection model', 'Define compliance scoring logic'] },
  { n: 'API Gateway',         ic: '⚡', d: 'FastAPI gateway: auth, rate limiting, request routing.',
    tasks: ['Define API spec (OpenAPI)', 'Design RBAC model', 'Plan JWT + MFA auth flow', 'Define rate limiting strategy', 'Document API versioning policy'] },
  { n: 'Dashboard Frontend',  ic: '🖥', d: 'React + TypeScript security operations dashboard.',
    tasks: ['Design information architecture', 'Create component library plan', 'Define data visualization strategy', 'Plan WebSocket real-time updates', 'Define accessibility requirements'] },
  { n: 'Notification Service',ic: '📨', d: 'Email, in-app, webhook, digest scheduling.',
    tasks: ['Define notification trigger taxonomy', 'Design digest schedule logic', 'Plan email template system', 'Define webhook payload schema'] },
];
